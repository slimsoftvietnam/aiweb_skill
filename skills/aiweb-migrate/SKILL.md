...
[Domain cũ]
    ...
    → (5) Import tài nguyên domain cũ (ảnh, CSS, JS, font) → Migration API + JSON catalog
...
**Catalog UI (read-only):** `aiweb_core/data/migration/{domain}.json` — tab **Tài nguyên migrate** tại Quản lý ảnh. Chỉ Migration API ghi JSON + DB `migration_assets`. Admin **không** upload/xóa file migrate.

**Reuse:** Cùng `source_url` giữa nhiều landing → import một lần, `import_asset` lần sau trả `skipped: true` + `local_path` cũ; `rewrite_assets` dùng chung map.

### B.1.1. Lệnh slimcrm.vn (full landing)
...
Kỳ vọng: ~37 landing, hàng trăm asset (css/js/ảnh). Verify tab **Tài nguyên migrate** và URL public `/{slug}` (vd. `/home`, `/tinhnang`).
...
- Mở URL công khai landing: `/{slug}` (slimcrm migrate) hoặc `/p/{slug}` tùy cấu hình site
